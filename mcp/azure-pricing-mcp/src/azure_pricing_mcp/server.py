...
async def main() -> None:
    ...
    async with pricing_server:
        if args.transport == "http":
            # Use Streamable HTTP transport (MCP protocol 2024-11-05+)
            # Clients connect to POST /mcp  (e.g. http://localhost:8080/mcp)
            import contextlib
            ...
            logger.info(f"Starting Streamable HTTP MCP server on {args.host}:{args.port} (endpoint: /mcp)")
